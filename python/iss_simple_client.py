...
requests = {
    ...
    'candles_day': 'http://iss.moex.com/iss/history/engines/%(engine)s/markets/%(market)s/boards/%(board)s/securities/%(ticker)s/candles.json?from=%(start_date)s&till=%(stop_date)s&interval=24',
    'names': 'http://iss.moex.com/iss/history/engines/%(engine)s/markets/%(market)s/boards/%(board)s/securities.json?'
}
...
class MicexISSClient:
    ...
    def get_all_by_board(self, engine, market, board):
        """ Get and parse historical data on all the securities at the
        given engine, market, board
        """
        url = requests['names'] % {'engine': engine,
                                   'market': market,
                                   'board': board}
    ...
    def get_history_securities_by_range(self, engine, market, board, ticker, start_date, stop_date):
        """ Get and parse historical data on all the securities at the
        given engine, market, board
        """
        url = requests['candles_day'] % {'engine': engine,
                                         'market': market,
                                         'board': board,
                                         'ticker': ticker,
        ...
        while cnt > 0:
            ...
            result = []
            for sec in jdata:
                result.append((sec[dateIdx], sec[secIdx],
                               del_null(sec[closeIdx]),
                               del_null(sec[tradesIdx])))
            # we return pieces of received data on each iteration
            # in order to be able to handle large volumes of data
